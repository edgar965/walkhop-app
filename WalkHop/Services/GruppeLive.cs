...
public static class GruppeLive
{
    ...
    private static string? _fantasieName;

    /// <summary>Anzeigename in der Gruppe: zuvor gewählter Name, sonst der Vorschlag (<see cref="StandardName"/>).</summary>
    public static string Anzeigename()
        => !string.IsNullOrWhiteSpace(Einst.GruppenName) ? Einst.GruppenName.Trim() : StandardName();

    /// <summary>Default-Namensvorschlag: Login-/Kontoname, sonst E-Mail, sonst ein Fantasiename
    /// „Wandie&lt;Zahl&gt;" (einmal pro Sitzung stabil, damit der gesendete Name nicht springt).</summary>
    public static string StandardName()
    {
        if (!string.IsNullOrWhiteSpace(Auth.Name)) return Auth.Name.Trim();
        if (!string.IsNullOrWhiteSpace(Auth.Email)) return Auth.Email.Trim();
        return _fantasieName ??= "Wandie" + RandomNumberGenerator.GetInt32(10, 100);
    }
