...
public class Lokalisierung : INotifyPropertyChanged
{
    ...
    private Lokalisierung()
    {
        ...
        WendeFormatCultureAn(_sprache);   // Zahlen-/Dezimalformat an die App-Sprache binden (nicht an die Geräte-Culture)
    ...
    /// <summary>Bindet die Zahlen-/Dezimal-/Datumsformatierung zentral an die APP-Sprache statt an die
    /// Geräte-Culture: de → de-DE (Dezimal-Komma „1,5 km"), en → en-US (Dezimal-Punkt „1.5 km"). Das wirkt
    /// einheitlich für die XAML-StringFormat-Bindings (z. B. „{0:0.0} km") UND für <see cref="L.T(string, object[])"/>
    /// (string.Format), weil beide die CurrentCulture verwenden. DefaultThreadCurrent* deckt zusätzlich neu
    /// gestartete Threads (Routing/IO) ab.</summary>
    private static void WendeFormatCultureAn(string sprache)
    {
        var culture = sprache == "de" ? new CultureInfo("de-DE") : new CultureInfo("en-US");
    ...
    private void Setze(string sprache)
    {
        ...
        _sprache = sprache;
        WendeFormatCultureAn(sprache);   // Format-Culture beim Sprachwechsel mitziehen
